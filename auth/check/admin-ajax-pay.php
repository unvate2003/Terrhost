...
$sel = mysqli_query($conn, "SELECT count(*) AS allcount FROM `san-pham-da-ban`");
$records = @mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];
## Total number of records with filtering
$sel = mysqli_query($conn,"SELECT count(*) AS allcount FROM `san-pham-da-ban` WHERE 1 ".$searchQuery);
$records = @mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];
## Fetch records
$empQuery = "SELECT * FROM `san-pham-da-ban` WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
...
while ($row = @mysqli_fetch_assoc($empRecords)) {
   ...
   // $active = $row['active'];
    ...
    $data[] = array(
            ...
            "trangthai"=>'<small>'.$nguoimua.'</small>',
    		"xoa"=>$xoa
    	);
}
## Response
