...
$sel = mysqli_query($conn, "SELECT count(*) AS allcount FROM `lich-su-mua`");
$records = @mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];
## Total number of records with filtering
$sel = mysqli_query($conn,"SELECT count(*) AS allcount FROM `lich-su-mua` WHERE 1 ".$searchQuery);
$records = @mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];
## Fetch records
$empQuery = "SELECT * FROM `lich-su-mua` WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
