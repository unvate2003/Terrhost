...
if ($_POST && $_POST['type'] == 'xoa-san-pham' && isset($_POST['products']) && isset($_POST['category']) && isset($_POST['subcategory'])) {
    $products = json_decode($_POST['products'], true);
    ...
    foreach ($products as $product) {
        ...
        if ($check) {
        $query = mysqli_query($conn,"DELETE FROM `san-pham-chua-ban` WHERE `text` = '$text' AND `subcate` = '$subcate' AND cate = '$cate' ");
        }
    ...
    echo json_encode(['deleteCount' => $deleteCount, 'errorCount' => $errorCount, 'success' => true]);
    exit;
}
      ...
      <div class="container-fluid">
        ...
        <div class="col-md-12">
          ...
          <div class="form-group text-center">
            <button type="button" class="btn btn-primary" id="them-san-pham">Thêm ngay <span id="addloading"></span></button>
            <button type="button" class="btn btn-danger" id="xoa-san-pham">Xóa ngay <span id="deleteloading"></span></button>
          </div>
          <hr>
          <div class="form-group text-center">
...
$(function() {
    $('#xoa-san-pham').click(function() {
        ...
        $('#deleteloading').html('<i class="fa fa-spinner fa-spin"></i>');

        var products = list.split("\n");
        $('#total').html(products.length);
        
        $.post('#', { type: 'xoa-san-pham', products: JSON.stringify(products), category, subcategory }, 'json')
        .done(function(res) {
            try {
                var result = JSON.parse(res);
                $('#success').html(result.deleteCount);
                $('#error').html(result.errorCount);

                toastr.success('Xóa thành công ' + result.deleteCount + ' sản phẩm', 'Thông báo');
            } catch (e) {
...
$(function() {
    $('#them-san-pham').click(function() {
        ...
        $('#addloading').html('<i class="fa fa-spinner fa-spin"></i>');
