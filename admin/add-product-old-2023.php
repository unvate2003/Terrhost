...
if($_POST && $_POST['type'] == 'them-san-pham' && isset($_POST['text']) && isset($_POST['category']) && isset($_POST['subcategory'])){
    ...
    $check2 = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM `san-pham-da-ban` WHERE `text` = '$text' AND `subcate` = '$subcate' AND cate = '$cate'"));
    if($check){
...
function xoasp(i){
       if(i >= total){
        ...
        var text = split[i];
        $.post('#',{type:'xoa-san-pham',text,subcategory,category},'json')
        .done((res)=>{
